...
// Device mockup component for white theme
  ...
  const mockups = {
    'Digital Billboard': (
      <div className="relative w-full h-full">
        {/* Billboard Frame */}
        <div className="relative w-full h-full bg-gradient-to-br from-gray-200 to-gray-300 rounded-xl p-2 shadow-lg">
          <div className="relative w-full h-full bg-black rounded-lg overflow-hidden">
            ...
            >
              <div className="text-center p-4">
                <div className="text-4xl font-black text-white mb-2 drop-shadow-lg">{content.letter}</div>
            ...
            {isActive && (
              ...
              >
                <div className="px-2 py-1 bg-red-500 text-white text-xs font-bold rounded shadow-lg">LIVE</div>
              </motion.div>
            )}
          </div>
        </div>
        {/* Stand */}
        <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-8 h-8 bg-gradient-to-b from-gray-400 to-gray-600 rounded-b shadow-lg" />
      </div>
    ),
    'Mobile Ads': (
      <div className="relative w-full h-full flex items-center justify-center">
        {/* Phone Frame */}
        <div className="relative w-32 h-56 bg-gradient-to-br from-gray-200 to-gray-300 rounded-3xl p-1 shadow-xl">
          <div className="relative w-full h-full bg-black rounded-3xl overflow-hidden">
            ...
            <div className={`absolute inset-0 bg-gradient-to-br ${content.color} flex items-center justify-center`}>
              ...
              >
                <div className="text-3xl font-black text-white mb-1 drop-shadow-lg">{content.letter}</div>
                <div className="text-xs text-white/90 font-semibold">{content.mediaType}</div>
              </motion.div>
            </div>
            {/* Home Bar */}
            <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 w-12 h-1 bg-white/40 rounded-full" />
          </div>
        </div>
      </div>
    ),
    'LED Screen': (
      <div className="relative w-full h-full">
        <div className="relative w-full h-full bg-black rounded-lg overflow-hidden border-4 border-gray-300 shadow-xl">
          <div className={`absolute inset-0 bg-gradient-to-br ${content.color}`}>
            ...
            <div className="relative z-10 flex items-center justify-center h-full">
              <div className="text-center">
                <div className="text-5xl font-black text-white mb-2 drop-shadow-lg">{content.letter}</div>
                <div className="text-sm text-white/95 font-bold">{content.title}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    default: (
      <div className="relative w-full h-full">
        <div className={`relative w-full h-full bg-gradient-to-br ${content.color} rounded-xl flex items-center justify-center overflow-hidden shadow-lg`}>
          <motion.div
            animate={isActive ? {
              scale: [1, 1.1, 1],
              rotate: [0, 3, 0]
          ...
          >
            <div className="text-6xl font-black text-white mb-2 drop-shadow-lg">{content.letter}</div>
            <div className="text-sm text-white/90 font-semibold">{type}</div>
          </motion.div>
        </div>
      </div>
    )
...
export default function FrameworkAdvertisingWhite() {
  ...
  return (
    <section className="py-20 lg:py-32 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Subtle Orbs */}
        <motion.div
          className="absolute top-1/4 right-1/4 w-96 h-96 bg-publixy-teal/5 rounded-full blur-3xl"
          animate={{
            scale: [1, 1.3, 1],
            opacity: [0.2, 0.4, 0.2],
          }}
        ...
        />
        <motion.div
          className="absolute bottom-1/3 left-1/3 w-96 h-96 bg-publixy-gold/5 rounded-full blur-3xl"
          animate={{
            scale: [1.3, 1, 1.3],
            opacity: [0.4, 0.2, 0.4],
          }}
      ...
      <div className="relative z-10 container mx-auto px-6">
        ...
        <div className="text-center mb-12">
          <motion.div
            ...
            className="mb-4"
          >
            <span className="inline-block px-6 py-3 text-sm font-black tracking-wider text-white uppercase bg-gradient-to-r from-publixy-gold to-publixy-yellow rounded-full shadow-lg">
          ...
          {frameworkData.map((item, index) => (
            ...
            >
              <motion.div
                ...
                className={`relative h-[400px] rounded-2xl overflow-hidden border transition-all duration-500 bg-white shadow-lg ${
                  index === activeCard 
                    ? 'border-publixy-gold/40 shadow-2xl shadow-publixy-gold/10' 
                ...
                <div className="absolute inset-0 p-6">
                  ...
                  <div className="relative z-10">
                    <h3 className="text-2xl font-bold text-gray-900 mb-2">
                      <span className={`inline-block w-8 h-8 rounded bg-gradient-to-br ${item.color} text-center leading-8 text-white font-black mr-2 shadow-lg`}>
                        {item.letter}
                      </span>
                      {item.title}
                    </h3>
                    <p className="text-publixy-gold text-sm font-bold mb-2">{item.subtitle}</p>
                    ...
                    {index === activeCard && (
                      ...
                      >
                        {Object.entries(item.metrics).map(([key, value]) => (
                          <div key={key} className="bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                            <div className="text-lg font-black text-publixy-teal">{value}</div>
                            <div className="text-xs text-gray-500 capitalize font-semibold">{key}</div>
                          </div>
                        ))}
                      </motion.div>
                    )}
          ...
          {frameworkData.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                index === activeCard 
                  ? 'w-8 bg-gradient-to-r from-publixy-gold to-publixy-yellow shadow-lg' 
