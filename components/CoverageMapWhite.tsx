...
const CityInfo = ({ city, onClose }: { city: typeof cities[0], onClose: () => void }) => (
  <motion.div
    ...
    className="absolute z-50 bg-white backdrop-blur-xl border border-gray-200 rounded-2xl p-6 min-w-[280px] shadow-2xl"
    style={{
      left: `${city.x}%`,
      top: `${city.y - 15}%`,
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-4 mb-4">
      <div>
        ...
        <div className="text-xs text-gray-600 font-semibold">Pantallas</div>
      </div>
      <div>
        <div className="text-2xl font-black text-publixy-gold">{city.population}</div>
        <div className="text-xs text-gray-600 font-semibold">Población</div>
      </div>
    </div>
    
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
      <div className="flex items-center justify-between">
        <span className="text-sm text-gray-700 font-semibold">Crecimiento</span>
        <span className="text-sm font-bold text-green-600">{city.growth}</span>
      </div>
...
export default function CoverageMapWhite() {
  ...
  return (
    <section className="py-20 lg:py-32 bg-gradient-to-b from-gray-50 via-white to-gray-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <motion.div
          className="absolute top-1/4 left-1/4 w-72 h-72 bg-publixy-teal/5 rounded-full blur-3xl"
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.2, 0.4, 0.2],
          }}
        ...
        />
        <motion.div
          className="absolute bottom-1/4 right-1/4 w-72 h-72 bg-publixy-gold/5 rounded-full blur-3xl"
          animate={{
            scale: [1.2, 1, 1.2],
            opacity: [0.4, 0.2, 0.4],
          }}
      ...
      <div className="relative z-10 container mx-auto px-6">
        ...
        >
          <span className="inline-block px-6 py-3 text-sm font-black tracking-wider text-white uppercase bg-gradient-to-r from-publixy-teal to-publixy-gold rounded-full mb-6 shadow-lg">
            Cobertura Nacional
          </span>
        ...
        >
          {/* Mexico Map SVG */}
          <div className="relative w-full h-[600px] bg-white backdrop-blur-sm rounded-3xl border border-gray-200 overflow-hidden shadow-2xl">
            {/* Simplified Mexico Shape */}
            <svg viewBox="0 0 100 80" className="absolute inset-0 w-full h-full">
              <defs>
                <linearGradient id="mexicoGradientWhite" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#7BD3E2" stopOpacity="0.05" />
                  <stop offset="100%" stopColor="#EFBD4A" stopOpacity="0.05" />
                </linearGradient>
              </defs>
            ...
            {cities.map((city, index) => (
              <div key={city.name}>
                ...
                >
                  ...
                  {/* Main Dot */}
                  <div className={`relative w-4 h-4 rounded-full border-2 border-white shadow-lg transition-all duration-300 ${
                    activeCity === index 
                  ...
                  {/* Screen Count Badge */}
                  <motion.div
                    className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg font-semibold"
                    initial={{ y: 10 }}
                    whileHover={{ y: 0 }}
                  >
                ...
                {index < cities.length - 1 && (
                  ...
                  >
                    <div className="h-full bg-gradient-to-r from-publixy-teal/20 to-publixy-gold/20" />
                  </motion.div>
                )}
              </div>
            ))}
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-publixy-teal rounded-full shadow-lg" />
              <span className="text-sm text-gray-600 font-semibold">Ciudad Activa</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-publixy-gold rounded-full shadow-lg" />
              <span className="text-sm text-gray-600 font-semibold">Ciudad Seleccionada</span>
            </div>
            <div className="flex items-center gap-2">
              <motion.div
                className="w-4 h-4 bg-publixy-gold/40 rounded-full"
                animate={{
                  scale: [1, 1.5, 1],
                  opacity: [0.4, 0.8, 0.4]
                }}
              ...
              />
              <span className="text-sm text-gray-600 font-semibold">Transmisión en Vivo</span>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-16 max-w-4xl mx-auto">
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-center p-6 bg-white backdrop-blur-sm rounded-2xl border border-gray-200 shadow-lg"
          >
            <div className="text-4xl font-black text-publixy-teal mb-2">
              {cities.reduce((sum, city) => sum + city.screens, 0)}
            </div>
            <div className="text-gray-600 font-semibold">Total Pantallas</div>
          </motion.div>
          ...
          >
            ...
            <div className="text-gray-600 font-semibold">Ciudades</div>
          </motion.div>
          ...
          >
            ...
            <div className="text-gray-600 font-semibold">Población Total</div>
          </motion.div>
          ...
          >
            ...
            <div className="text-gray-600 font-semibold">Cobertura Nacional</div>
          </motion.div>
        </div>
      </div>
    </section>
